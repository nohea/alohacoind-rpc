...
function RpcClient(opts) {
  //console.log("RpcClient() ctor " + JSON.stringify(opts));
    
  opts = opts || {};
  this.host = opts.host || '127.0.0.1';
  this.port = opts.port || 9422;
...
function rpc(request, callback) {
  ...
  var errorMessage = 'Alohacoin JSON-RPC: ';
  ...
  var req = this.protocol.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
        var exceededError = new Error('Alohacoin JSON-RPC: ' + buf.toString('utf8'));
